...
```{r}
tm_shape(biodiversity) + 
  tm_polygons(col = 'habitat_frag')
```

```{r}
# new lm with all variables
pop_n_frag_lm <- lm(sp_richness ~ kg_prd_used + pop_density + habitat_frag + land_use, biodiversity)

summary(pop_n_frag_lm)
```
...
summary(lm(habitat_frag ~ land_use, biodiversity))
```
